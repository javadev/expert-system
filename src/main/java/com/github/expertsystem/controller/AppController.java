...
public class AppController {
    ...
    public void onNext(
            DefaultListModel listModelExperts,
    ...
    public void onSaveMarks(int indexCriterion, DefaultTableModel tableModel) {
        for (int i = 0; i < Experts.getExperts().size(); i++) {
            for (int j = 0; j < Objects.getObjects().size(); j++) {
                String cellValue =
                        tableModel.getValueAt(i, j + 1).toString().replace(',', '.').trim();
                if (checkString(cellValue)) {
                    float value = Float.parseFloat(tableModel.getValueAt(i, j + 1).toString());
                    if (value >= 0) {
                        Marks.setMark(
                                Experts.findExpert(i),
                                Objects.findObj(j),
                                Criterions.findCriterion(indexCriterion),
                                value);
                    } else {
                        String msg =
                                "Значение оценки должно быть положительным: эксперт "
                                        + Experts.findExpert(i).getName()
                                        + ", объект "
                                        + Objects.findObj(j).getName()
                                        + "!";
                        JOptionPane.showMessageDialog(
                                null, msg, "Ошибка", JOptionPane.ERROR_MESSAGE);
                    }
                } else {
                    String msg =
                            "Неверно введено значение оценки: эксперт "
                                    + Experts.findExpert(i).getName()
                                    + ", объект "
                                    + Objects.findObj(j).getName()
                                    + "!";
    ...
    public boolean onAddExpert(
            String textFieldExperts,
            String textFieldExpertsK,
            DefaultListModel listModelExperts,
            DefaultListModel listModelExpertsK) {
        ...
        if (!listModelExperts.contains(field)) {
            if (!field.equals("")) {
                if (checkString(fieldK)) {
                                    ...
                                    Float.parseFloat(
                                            ...
                                            - (Float.parseFloat(
                                                            listModelExpertsK
                                                                    ...
                                                                    .replace(',', '.'))
                                                    * val);
                            listModelExpertsK.setElementAt(
                                    formatter.format(value).replace(',', '.'), i);
                        }
                    } else {
                        JOptionPane.showMessageDialog(
                                null,
                                "Коэффициент компетентности эксперта должен быть в пределах от 0 до 1!",
                                "Ошибка",
                    ...
                    JOptionPane.showMessageDialog(
                            null,
                            "Неверно введен коэффициент компетентности эксперта!",
                            "Ошибка",
                            JOptionPane.ERROR_MESSAGE);
                    return false;
                }
            ...
            JOptionPane.showMessageDialog(
                    null,
                    "Эксперт с данным именем уже существует!",
                    "Ошибка",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        }
    ...
    public void onDelExpert(
        ...
        if (selectedIndex > -1) {
            ...
            float delValue =
                    Float.parseFloat(
                            listModelExpertsK.get(selectedIndex).toString().replace(',', '.'));
            listModelExpertsK.removeElementAt(selectedIndex);
            for (int i = 0; i < listModelExpertsK.getSize(); i++) {
                float value =
                        (Float.parseFloat(listModelExpertsK.get(i).toString().replace(',', '.'))
                                        * 100.0f)
                                / ((1.0f - delValue) * 100.0f);
    ...
    public Object[][] getDataForMarksTable(
            int indexOfCriterion, DefaultListModel listModelExperts) {
    ...
    public String getResultMethod1() {
        return Objects.findObj(method1.getTheBestObject())
                + " (№ "
                + (method1.getTheBestObject() + 1)
                + ")";
    }
    ...
    public String getResult1Method5() {
        ...
        for (int cr = 0; cr < Criterions.getSize(); cr++) {
            ...
            for (int[] pair : method5.getSpearmanResultMax(cr)) {
                res +=
                        Experts.findExpert(pair[0]).getName()
                                + " и "
                                + Experts.findExpert(pair[1]).getName()
                                + "\n";
            }
