...
public class Method3 {
    ...
    public static final float[][] X_TABLE  = {{ 6.6f,   5.0f,   3.8f,   0.0039f,  0.00098f, 0.00016f }, // k=1
    ...
    private void rangs2Calculate() {
        ...
        outPut.println();
        String str = "";
        outPut.println("Расчет нормированных рангов, сумм рангов и оценок математического ожидания");
        outPut.println();
    ...
    private void r2AndDCalculate() {
        ...
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
            ...
            outPut.println("Дисперсия: D = " + r2Sums[cr] + " / (" + Objects.getSize() + " - 1) = " + formatter.format(d[cr]));
            outPut.println();
        }
        outPut.println();
    }
    ...
    private void dMaxCalculate() {
        dMax = (Experts.getSize() * Experts.getSize() *
                ((float)Math.pow(Objects.getSize(), 3) - Objects.getSize()) /
                (12 * (Objects.getSize() - 1)));
        outPut.println("Максимальная дисперсия: Dmax = " + Experts.getSize() + "^2 * (" + formatter.format(Math.pow(Objects.getSize(), 3)) +
                " - " + Objects.getSize() + ") / (12 * (" + Objects.getSize() + " - 1)) = " + formatter.format(dMax));
        outPut.println();
    }
    ...
    private void withoutRangsCalculate() {
        ...
        dMaxCalculate();
        outPut.println();
        outPut.println("Расчет коэффициента Спирмена без учета связанных рангов ( формула: W = D / Dmax )");
        outPut.println();
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
            outPut.println("Расчет по показателю " + Criterions.findCriterion(cr).getName().toUpperCase());
            w1WithoutRangs[cr] = (12 * r2Sums[cr]) /
                    ((Experts.getSize() * Experts.getSize()) *
                    ((float)Math.pow(Objects.getSize(), 3) - Objects.getSize()));
            w2WithoutRangs[cr] = d[cr] / dMax;
            outPut.println("W = " + d[cr] + " / " + formatter.format(dMax) + " = " + formatter.format(w2WithoutRangs[cr]));
            outPut.println();
        }
    }
    ...
    private void repeatsCalculate() {
        ...
        String str = "";
        outPut.println();
        outPut.println("Расчет показателя связанных рангов в j-й ранжировке ( формула: Tj = Σk (hk^3 - hk) )");
        outPut.println();
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
            outPut.println("Расчет по показателю " + Criterions.findCriterion(cr).getName().toUpperCase());
            ...
            for(int ex = 0; ex < Experts.getSize(); ex++) {
                ...
                for (int ob = 0; ob < Objects.getSize(); ob++) {
                    ...
                    str = str + "(" + repeats.get(cr)[ex][ob] + "^3" + " - " + repeats.get(cr)[ex][ob] + ") + ";
                }
    ...
    private void withRangsCalculate() {
        repeatsCalculate();
        outPut.println();
        outPut.println("Расчет коэффициента Спирмена (W) и критерия согласования Пирсона (X) с учетом связанных рангов");
        outPut.println();
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
            outPut.println("Расчет по показателю " + Criterions.findCriterion(cr).getName().toUpperCase());
            wWithRangs[cr] = (12.0f * r2Sums[cr]) / (Experts.getSize() * Experts.getSize() *
                    ((float) Math.pow(Objects.getSize(), 3) - Objects.getSize()) -
                    Experts.getSize() * tSums[cr]);
            xWithRangs[cr] = (12.0f * r2Sums[cr]) / ((Experts.getSize() * Objects.getSize() *
                    (Objects.getSize() + 1.0f)) - ((1.0f / (Objects.getSize() - 1.0f)) * tSums[cr]));
    ...
    private void hMaxCalculate() {
        ...
        outPut.println();
        outPut.println("Максимальная энтропия: Hmax = n * log10(n) = " + Objects.getSize() + " * " +
                formatter.format(Math.log10(Objects.getSize())) + ") = " + formatter.format(hMax));
    }
    ...
    private void entropyCalculate() {
        ...
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
            outPut.println("Расчет по показателю " + Criterions.findCriterion(cr).getName().toUpperCase());
            outPut.println();
            outPut.println("Расчет оценок вероятностей j-го ранга, присваиваемого i-му объекту ( формула: Pij = Mij / m )");
            ...
            for (int ob1 = 0; ob1 < Objects.getSize(); ob1++) {
                for (int ob2 = 0; ob2 < Objects.getSize(); ob2++) {
                    ...
                    outPut.println("P" + (ob1 + 1) + "," + (ob2 + 1) + " = " + numberEntropy(cr, ob1, ob2) + " / " +
                            Experts.getSize() + " = " + formatter.format(p));
                    if(p != 0) {
    ...
    public void resultCalculate() {
        result = "Анализ результатов расчетов дисперсионных коэффициентов конкордации\n\n" +
                "Квантиль распределения хи-квадрат для " + (Objects.getSize() - 1) +
                " степеней свободы \nи уровне значимости а = " + formatter.format(a) + " равен " +
                formatter.format(x) + "\n";
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
            ...
            if(x < xWithRangs[cr]) {
                result += formatter.format(x) + " < " + formatter.format(xWithRangs[cr]) +
        ...
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
            ...
            if(isTwoGroupsOfExperts(cr)) {
                ...
                result += "Разделение экспертов на 2 группы с противоположными точками \nзрения не выявлено.\n";
            }
        }
