...
public class Method5 {
    ...
    private void pairRangsCalculate() {
        ...
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
            ...
            for(int ex = 0; ex < Experts.getSize(); ex++) {
                ...
                for(int j = 0; j < Objects.getSize(); j++) {
                    outPut.println("R" + (j + 1) + " = " + formatter.format(pairRangs.get(cr).get(ex)[j]));
                }
                outPut.println();
            }
            outPut.println();
    ...
    private void spearmanCalculate() {
        String str = "";
        outPut.println("Расчет коэффициентов корреляции Спирмена ( формула: ρkt = 1 - (6 / (n^3 - n)) * Σj ((Rkj - Rtj)^2) )");
        outPut.println();
        int sum;
        float max, min;
        for(int cr = 0; cr < Criterions.getSize(); cr++) {
                ...
                for(int ex2 = 0; ex2 < Experts.getSize(); ex2++) {
                    ...
                    if(ex1 != ex2) {
                        ...
                        for(int ob = 0; ob < Objects.getSize(); ob++) {
                            sum += (Math.pow((pairRangs.get(cr).get(ex1)[ob] - pairRangs.get(cr).get(ex2)[ob]), 2));
                        }
                        spearman.get(cr)[ex1][ex2] = (float) (1.0f - ((6.0f / (Math.pow(Objects.getSize(), 3) - Objects.getSize())) * sum));
                        str = str + "1 - (6 / " + formatter.format(Math.pow(Objects.getSize(), 3)) + " - " +
                                Objects.getSize() + ") * " + formatter.format(sum) + " = " +
    ...
    private void kendallCalculate() {
        String str = "";
        outPut.println("Расчет коэффициентов корреляции Кендалла " +
            ...
            for(int ex1 = 0; ex1 < Experts.getSize(); ex1++) {
                for(int ex2 = 0; ex2 < Experts.getSize(); ex2++) {
                    ...
                    if(ex1 != ex2) {
                        str = "τ" + (ex1 + 1) + "," + (ex2 + 1) + " = ";
                        for(int ob = 0; ob < Objects.getSize(); ob++) {
                            sign = sign(pairRangs.get(cr).get(ex1)[ob] - pairRangs.get(cr).get(ex1)[Objects.getSize() - 1])
                                    * sign(pairRangs.get(cr).get(ex2)[ob] - pairRangs.get(cr).get(ex2)[Objects.getSize() - 1]);
                            sum += sign;
                        }
                        kendall.get(cr)[ex1][ex2] = (2.0f / (Objects.getSize() * (Objects.getSize() - 1.0f))) * sum;
                        str = str + "2 / (" + Objects.getSize() + " * (" + Objects.getSize() + " - 1)) * " +
                                formatter.format(sum) + " = " + formatter.format(kendall.get(cr)[ex1][ex2]);
                        outPut.println(str);
